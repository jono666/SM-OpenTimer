...
/*
	...
	Recording data...
*/
stock bool SaveRecording( int id, int run, int style, int mode, ArrayList &hRec, float flTime, int jumps, int strafes, char szName[MAX_NAME_LENGTH], char szSteam[MAX_ID_LENGTH] )
{
...
stock bool LoadRecording( ArrayList &hRec, int &tickcount, int id, int iRun, int iStyle, int iMode )
{
	...
	ReadFileCell( hFile, iTemp, 4 );
	
	if ( iTemp != MAGIC_NUMBER )
	{
	...
	if ( iTemp != RoundFloat( g_flTickRate ) )
	{
		LogError( CONSOLE_PREFIX..."Recording tickrate differs from server's tickrate! (Recording: %i / Server: %.0f)", iTemp, g_flTickRate );
		return false;
	}
	...
	float flTime;
	if ( ReadFileCell( hFile, view_as<int>( flTime ), 4 ) == -1 )
	{
		LogError( CONSOLE_PREFIX..."Tried to read from a recording with no time specified in the header(?)" );
		return false;
	}
