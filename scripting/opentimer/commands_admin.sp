public Action Command_Admin_ZoneEnd( int client, int args )
{
	...
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You haven't even started to build!" );
	...
	if ( (zone == ZONE_START || zone == ZONE_END) && (g_bZoneExists[ZONE_START] && g_bZoneExists[ZONE_END]) )
	{
		...
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is now available!", g_szRunName[NAME_LONG][RUN_MAIN] );
	}
	else if ( (zone == ZONE_BONUS_1_START || zone == ZONE_BONUS_1_END) && (g_bZoneExists[ZONE_BONUS_1_START] && g_bZoneExists[ZONE_BONUS_1_END]) )
	{
		...
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is now available!", g_szRunName[NAME_LONG][RUN_BONUS1] );
	}
	else if ( (zone == ZONE_BONUS_2_START || zone == ZONE_BONUS_2_END) && (g_bZoneExists[ZONE_BONUS_2_START] && g_bZoneExists[ZONE_BONUS_2_END]) )
	{
		...
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is now available!", g_szRunName[NAME_LONG][RUN_BONUS2] );
	}
	...
	if ( zone == ZONE_CP )
	{
		PRINTCHATV( client, client, CHAT_PREFIX..."Created "...CLR_TEAM..."%s"...CLR_TEXT..." for "...CLR_TEAM..."%s"...CLR_TEXT..." successfully!", g_szZoneNames[zone], g_szRunName[NAME_LONG][run] );
	}
	else PRINTCHATV( client, client, CHAT_PREFIX..."Created "...CLR_TEAM..."%s"...CLR_TEXT..." successfully!", g_szZoneNames[zone] );
...
public Action Command_Admin_ZoneCancel( int client, int args )
{
	...
	if ( g_iBuilderZone[client] == ZONE_INVALID )
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You have no zone to cancel!" );
		return Plugin_Handled;
	}
...
public Action Command_Admin_ZoneEdit_SelectCur( int client, int args )
{
	...
	PRINTCHAT( client, client, CHAT_PREFIX..."Sorry, couldn't find zones." );
