...
public int Handler_Hud( Menu mMenu, MenuAction action, int client, int item )
{
	...
	if ( StrEqual( szItem, "vm" ) )
	{
		if ( g_fClientHideFlags[client] & HIDEHUD_VM )
		{
			...
			PRINTCHAT( client, client, CHAT_PREFIX..."Restored viewmodel!" );
		}
		else
		{
			g_fClientHideFlags[client] |= HIDEHUD_VM;
			...
			PRINTCHAT( client, client, CHAT_PREFIX..."Your viewmodel is now hidden!" );
		}
	}
	else if ( StrEqual( szItem, "zmsg" ) )
	{
		if ( g_fClientHideFlags[client] & HIDEHUD_ZONEMSG )
		{
			...
			PRINTCHAT( client, client, CHAT_PREFIX..."Zone messages show up again!" );
		}
		else
		{
			g_fClientHideFlags[client] |= HIDEHUD_ZONEMSG;
			
			PRINTCHAT( client, client, CHAT_PREFIX..."Zone messages are now off." );
		}
	}
	...
	else if ( StrEqual( szItem, "cpi" ) )
	{
		if ( g_fClientHideFlags[client] & HIDEHUD_CPINFO )
		{
			...
			PRINTCHAT( client, client, CHAT_PREFIX..."Checkpoint information is back!" );
		}
		else
		{
			g_fClientHideFlags[client] |= HIDEHUD_CPINFO;
			
			PRINTCHAT( client, client, CHAT_PREFIX..."Checkpoint information is now hidden!" );
		}
	}
#if !defined CSGO
	...
	else if ( StrEqual( szItem, "zon" ) )
	{
		if ( g_fClientHideFlags[client] & HIDEHUD_SHOWZONES )
		{
			...
			PRINTCHAT( client, client, CHAT_PREFIX..."Other zones are now hidden!" );
		}
		else
		{
			g_fClientHideFlags[client] |= HIDEHUD_SHOWZONES;
			
			PRINTCHAT( client, client, CHAT_PREFIX..."Other zones are now shown!" );
		}
	}
#if defined CHAT
...
public int Handler_Vote( Menu mMenu, MenuAction action, int client, int index )
{
	...
	{
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%N"...CLR_TEXT..." changed their vote to "...CLR_TEAM..."%s"...CLR_TEXT..."!", client, szMap );
	}
	else
	{
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%N"...CLR_TEXT..." voted for "...CLR_TEAM..."%s"...CLR_TEXT..."!", client, szMap );
	}
...
public Action Command_Practise_GotoPoint( int client, int args )
{
	...
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You have to be in practice mode! ("...CLR_TEAM..."!prac"...CLR_TEXT...")" );
		return Plugin_Handled;
	}
	...
	if ( args != 0 )
	{
		...
		{
			PRINTCHATV( client, client, CHAT_PREFIX..."Invalid argument! (1-%i)", len );
			return Plugin_Handled;
		}
...
public int Handler_Empty( Menu mMenu, MenuAction action, int client, int item ) { return 0; }