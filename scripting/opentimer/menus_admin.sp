...
public Action Command_Admin_ZoneStart( int client, int args )
{
	...
	if ( g_iBuilderZone[client] != ZONE_INVALID )
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You've already started to build a zone!" );
		return Plugin_Handled;
	}
...
public Action Command_Admin_ZoneEdit( int client, int args )
{
	...
	if ( g_iBuilderZone[client] != ZONE_INVALID )
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You are still making a zone!" );
		return Plugin_Handled;
	}
	...
	if ( !len )
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."There are no zones to change!" );
...
public Action Command_Admin_ZonePermissions( int client, int args )
{
	...
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."You haven't chosen a zone to edit!" );
		return Plugin_Handled;
	}
	...
	mMenu.AddItem( "m", ( flags & ZONE_ALLOW_MAIN )		? "Allow Main: ON" : "Allow Main: OFF", ( g_bIsLoaded[RUN_MAIN] ) ? 0 : ITEMDRAW_DISABLED );
	mMenu.AddItem( "b1", ( flags & ZONE_ALLOW_BONUS1 )	? "Allow Bonus #1: ON" : "Allow Bonus #1: OFF", ( g_bIsLoaded[RUN_BONUS1] ) ? 0 : ITEMDRAW_DISABLED );
	mMenu.AddItem( "b2", ( flags & ZONE_ALLOW_BONUS2 )	? "Allow Bonus #2: ON" : "Allow Bonus #2: OFF", ( g_bIsLoaded[RUN_BONUS2] ) ? 0 : ITEMDRAW_DISABLED );
	
	mMenu.AddItem( "sc", ( flags & ZONE_ALLOW_NORMAL )	? "Allow Normal: ON" : "Allow Normal: OFF", ( zone == ZONE_BLOCKS ) ? 0 : ITEMDRAW_DISABLED );
	
	
	mMenu.AddItem( "sw", ( flags & ZONE_ALLOW_SW )				? "Allow Sideways: ON" : "Allow Sideways: OFF" );
	mMenu.AddItem( "w", ( flags & ZONE_ALLOW_W )				? "Allow W-Only: ON" : "Allow W-Only: OFF" );
	mMenu.AddItem( "rhsw", ( flags & ZONE_ALLOW_RHSW )			? "Allow Real-HSW: ON" : "Allow Real-HSW: OFF" );
	mMenu.AddItem( "hsw", ( flags & ZONE_ALLOW_HSW )			? "Allow HSW: ON" : "Allow HSW: OFF" );
	mMenu.AddItem( "vel", ( flags & ZONE_ALLOW_VELCAP )			? "Allow VelCap: ON" : "Allow VelCap: OFF", ( zone == ZONE_BLOCKS ) ? 0 : ITEMDRAW_DISABLED );
	mMenu.AddItem( "ad", ( flags & ZONE_ALLOW_A_D )				? "Allow A/D-Only: ON" : "Allow A/D-Only: OFF" );
	mMenu.AddItem( "ad", ( flags & ZONE_ALLOW_SCROLL )			? "Allow Legit: ON" : "Allow Legit: OFF", ( zone == ZONE_BLOCKS ) ? 0 : ITEMDRAW_DISABLED );
	
	mMenu.AddItem( "sc", ( flags & ZONE_VEL_NOSPEEDCAP )		? "VelCap Speedcap: OFF" : "VelCap Speedcap: ON", ( zone == ZONE_FREESTYLES ) ? 0 : ITEMDRAW_DISABLED );
...
public int Handler_ZonePermissions( Menu mMenu, MenuAction action, int client, int index )
{
	if ( action != MenuAction_Select ) return 0;
...
public int Handler_ZoneDelete( Menu mMenu, MenuAction action, int client, int zone )
{
	...
	if ( (zone == ZONE_START || zone == ZONE_END) && g_bIsLoaded[RUN_MAIN] )
	{
		...
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is no longer available for running!", g_szRunName[NAME_LONG][RUN_MAIN] );
	}
	else if ( (zone == ZONE_BONUS_1_START || zone == ZONE_BONUS_1_END) && g_bIsLoaded[RUN_BONUS1] )
	{
		g_bIsLoaded[RUN_BONUS1] = false;
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is no longer available for running!", g_szRunName[NAME_LONG][RUN_BONUS1] );
	}
	else if ( (zone == ZONE_BONUS_2_START || zone == ZONE_BONUS_2_END) && g_bIsLoaded[RUN_BONUS2] )
	{
		g_bIsLoaded[RUN_BONUS2] = false;
		PRINTCHATALLV( client, false, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." is no longer available for running!", g_szRunName[NAME_LONG][RUN_BONUS2] );
	}
	
	g_bZoneExists[zone] = false;
	PRINTCHATV( client, client, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." deleted.", g_szZoneNames[zone] );
...
public int Handler_ZoneDelete_S( Menu mMenu, MenuAction action, int client, int index )
{
	...
	int ent = g_hZones.Get( index, view_as<int>( ZONE_ENTINDEX ) );
	...
	if ( ent > 0 && IsValidEntity( ent ) )
	{
		...
		PRINTCHATV( client, client, CHAT_PREFIX...""...CLR_TEAM..."%s"...CLR_TEXT..." zone deleted.", g_szZoneNames[zone] );
	}
	else
	{
		PRINTCHATV( client, client, CHAT_PREFIX..."Couldn't remove "...CLR_TEAM..."%s"...CLR_TEXT..." zone!", g_szZoneNames[zone] );
...
public Action Command_Admin_ZoneDelete_CP( int client, int args )
{
	...
	if ( !len )
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."There are no checkpoints to delete!" );
		return Plugin_Handled;
	}
...
public int Handler_ZoneDelete_CP( Menu mMenu, MenuAction action, int client, int index )
{
	...
	int ent = g_hCPs.Get( index, view_as<int>( CP_ENTINDEX ) );
	...
	if ( ent > 0 && IsValidEntity( ent ) )
	{
		...
		// Erase them from the database.
		DB_EraseMapZone( ZONE_CP, id, g_iClientRun[client] );
		
		PRINTCHAT( client, client, CHAT_PREFIX..."Checkpoint deleted." );
	}
	else
	{
		PRINTCHAT( client, client, CHAT_PREFIX..."Couldn't remove checkpoint"...CLR_TEXT..."!" );
...
public int Handler_RunRecordsDelete_Confirmation( Menu mMenu, MenuAction action, int client, int index )
{
	...
	switch ( type )
	{
		...
		{
			PrintToServer( CONSOLE_PREFIX..."Removed %i recording files.", RemoveAllRecordings( run ) );
		}
